...
def get_final_rank(allele, list_predicted, dict_distr):
    list_out = []
    for score0 in list_predicted:
        rank0 = np.around(100 - percentileofscore(dict_distr[allele[4:]],score0),3)
        list_out.append(rank0)
    return list_out    

def ligand_prediction(allele, peptides):
    model_binding = import_model(model_dict, 'ligand/', 5)
    ...
    for allele in prediction_data.keys():
        ...
        scores = scoring(model_binding, mp_pairs1)
...
def mk_predictions(allele_peptides):
    ...
    for allele, peptides in allele_peptides.items():  
        ...
        all_prediction_data = ligand_prediction(allele, peptides)
        allele_idx = 0
        peptide_idx = 1
        el_score_idx = 2
        el_rank_idx = 3
...
def mk_predictions_datatable(sample_info, allele_peptides, sample_peptides1, binding_predictions, all_predictions): # add all predictions to the binding_predictions DataTable
    global tmp_folder
    for sample in sample_info:
        rows = []
        for allele, peptides in allele_peptides.items():
        ...
        for sample in samples:
            ...
            with open(tmp_folder + f'{sample}_ImmuneApp_predictions.tsv', 'w') as f:
                ...
                for allele in alleles:
                    for peptide in peptides:
                        ...
                        to_write = [allele, peptide] + [str(all_predictions[allele][peptide][k]) for k in keys]
...
def mk_cluster_with_gibbscluster(GIBBSCLUSTER, samples, sample_peptides, not_enough_peptides):
    ...
    for sample in samples:
        fname = Path(current_path, tmp_folder, f'{sample}_forgibbs.csv')
        ...
        for groups in range(1, n_groups+1):
            jds.append({"command" : f'{GIBBSCLUSTER} -f {fname} -P {groups}groups ' \
                    ...
                    "working_directory" : current_path + '/' + tmp_folder + 'gibbscluster/' + sample + '/unsupervised',
...
def mk_cluster_with_gibbscluster_by_allele(GIBBSCLUSTER, samples, sample_alleles, supervised_gibbs_directories, binding_predictions):
    ...
    for sample in samples:
        ...
        for allele, peps in allele_peps.items():
            fname = Path(current_path, tmp_folder, f"{allele}_{sample}_forgibbs.csv")
            ...
            if len(peps) < 20:
                ...
                for groups in range(1, n_groups+1):
                    jds.append({"command" : f'{GIBBSCLUSTER} -f {fname} -P {groups}groups ' \
                            ...
                            "working_directory" : current_path + '/' + tmp_folder +'gibbscluster/' + sample + '/' + allele,
                            "id" : f'gibbscluster_{groups}groups'})
...
def main(args):
    ...
    binding_predictions = mk_predictions_datatable(sample_info, allele_peptides, sample_peptides1, binding_predictions, all_predictions)                                                                    
...
if __name__ == '__main__':
    ...
    # python /collab2/hxu6/HLA_Prediction/python/Immunopeptidomics_analysis.py
    # -f sample_1.txt sample_2.txt sample_3.txt
    # -a HLA-A*01:01,HLA-A*25:01 HLA-B*08:01,HLA-B*18:01 HLA-B*08:01,HLA-B*18:01
    # -o jobs/test
